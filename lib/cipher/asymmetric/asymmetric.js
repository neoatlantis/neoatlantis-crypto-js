...
function AsymmetricCipher(algorithmName){
    ...
    function _digest(dataBuf){
        return 
            new root.hash(algorithmSuite.hashAlgorithm).hash(dataBuf).buffer;
    };

    function _newKey(){
        return new root.util.srand().bytes(algorithmSuite.secretLength);
    };
    ...
    function decrypt(ciphertextBuf){
        ...
        $.crypto.crypt().setKey(tempKey).decrypt(ciphertext);
    };
    ...
    this.setPublicKey = function(publicKeyBuf){
        if(!(
            ...
            publicKeyBuf.byteLength > 4
        ))
            throw new Error('invalid-public-key');

        var splitLen = new Uint32Array(publicKeyBuf.slice(0, 4))[0];

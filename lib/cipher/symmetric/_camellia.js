...
function cipherInterface(){
    ...
    function encrypt(buf){
        ...
        var input = new Uint8Array(buf), pBlock
            ret = new Uint8Array(input.length), cBlockLast = iv, cBlock;
        var i, j;

        for(i=0; i<input.length; i+=16){
            pBlock = input.slice(i, i+16);
            cBlock = core.encrypt(

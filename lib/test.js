...
test('PBKDF2-BLAKE2s', function(){
    var pbkdf2Buf = new crypto.hash('BLAKE2s', {length: 16}).pbkdf2(
    ...
    return crypto.util.buffer.equal(
        ...
        crypto.util.encoding(
            '8cf06475b3af278484b3e152b3c71291',
            'hex'
        ).toArrayBuffer()
    );
});

test('PBKDF2-WHIRLPOOL', function(){
    var pbkdf2Buf = new crypto.hash('WHIRLPOOL').pbkdf2(
        new Uint8Array([1,2,3]).buffer,
        new Uint8Array([4,5,6]).buffer,
        100,
        32
    );
    console.log(crypto.util.encoding(pbkdf2Buf).toHEX())
    return crypto.util.buffer.equal(
        pbkdf2Buf,
        crypto.util.encoding(
            'f654a7f7db0dc20c610c75e11c66d5eee43c0fb669bedff1439ec6dd5c599af8',
            'hex'
        ).toArrayBuffer()
    );
});

test('PBKDF2-RIPEMD160', function(){
    var pbkdf2Buf = new crypto.hash('RIPEMD160').pbkdf2(
        new Uint8Array([1,2,3]).buffer,
        new Uint8Array([4,5,6]).buffer,
        100,
        32
    ...
    return crypto.util.buffer.equal(
        pbkdf2Buf,
        crypto.util.encoding(
            '741990c203827200af14a60c64383c7fa481c22c518135d249075aac1d570875',
            'hex'
        ).toArrayBuffer()
    );
});

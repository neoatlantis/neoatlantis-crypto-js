...
function NCSC512(){
    ...
    function padding(srcBuffer, blockSize){
        ...
        var padlen = blockSize - srcBuffer.length % blockSize,
            padding = new $.node.buffer.Buffer(padlen);
        padding.fill(padlen);
        return $.node.buffer.Buffer.concat([srcBuffer, padding]);
    };

    function unpadding(buffer){
        var padlen = buffer.slice(-1)[0],
            expectedPadding = new $.node.buffer.Buffer(padlen);
        if(buffer.length < padlen) return false;
        expectedPadding.fill(padlen);
        if(buffer.slice(-padlen).toString() != expectedPadding) return false;

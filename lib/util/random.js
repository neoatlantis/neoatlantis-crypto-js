...
var touchArray = new Uint8Array(8),
    touchArrayCursor = 0,
    touchLastDuration = 0,
    touchLastTime = 0,
    touchThisDuration = 0,
...
function feedRandomPoolByTouch(){
    touchThisTime = new Date().getTime();
    touchThisDuration = touchThisTime - touchLastTime;
    touchArray[touchArrayCursor] = (touchThisDuration > touchLastDuration);
    touchArrayCursor += 1;
    if(touchArrayCursor > 7){
        touchArrayCursor = 0;
        randomPoolUpdate(
            (touchArray[0] << 0) +
            (touchArray[1] << 1) +
...
setInterval(feedRandomPoolByTouch, 50);
...
function srand(){
    ...
    this.bytes = function(count){
        ...
        for(var i=0; i<max; i++){
            getRandomBlock(pool);
            transArray = new Uint8Array(pool);
            for(j=0; j<64; j++){
                if(k >= count) break;
